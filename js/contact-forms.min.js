...
function isValidPhoneNumber(phone){const cleanPhone=phone.replace(/\D/g,'');if(cleanPhone.length<8||cleanPhone.length>15)return!1;const australianPatterns=[/^(\+?61\s?)?(\(?0[2-9]\)\s?|\(?0[2-9]\s?\)?)\s?\d{4}\s?\d{4}$/,/^(\+?61\s?)?\(?04\d{2}\)?\s?\d{3}\s?\d{3}$/,/^(\+?61\s?)?\(?1[38]\d{2}\)?\s?\d{3}\s?\d{3}$/],internationalPatterns=[/^(\+?1\s?)?\(?\d{3}\)?\s?[\d\s\-]{7,10}$/,/^(\+?\d{1,3}\s?)?\(?\d{2,4}\)?\s?[\d\s\-]{6,12}$/];return[...australianPatterns,...internationalPatterns].some(pattern=>pattern.test(phone))}
function validateField(fieldId,value,type,formType){const trimmedValue=value.trim();switch(type){case'name':if(!trimmedValue)return'Name is required';if(!validationPatterns.name.test(trimmedValue))return'Please enter a valid name (2-50 characters, letters only)';break;case'email':if(!trimmedValue)return'Email is required';if(!validationPatterns.email.test(trimmedValue))return'Please enter a valid email address';break;case'phone':if(!trimmedValue)return'Phone number is required';if(!validationPatterns.phone.test(trimmedValue))return'Phone number format is invalid';if(!isValidPhoneNumber(trimmedValue))return'Please enter a valid phone number (e.g., (03) 6169 2286 or 0412 345 678)';break;case'date':if(!trimmedValue)return'Date is required';const selectedDate=new Date(trimmedValue),today=new Date;if(today.setHours(0,0,0,0),selectedDate<today)return'Please select a future date';break;case'required':if(!trimmedValue)return'This field is required'}return null}
...
function toggleContactMethod(){const emailRadio=document.getElementById('issue-email-contact'),phoneRadio=document.getElementById('issue-phone-contact'),emailField=document.getElementById('issue-email-field'),phoneField=document.getElementById('issue-phone-field'),emailInput=document.getElementById('issue-email'),phoneInput=document.getElementById('issue-phone');emailRadio&&emailRadio.checked?(emailField.classList.remove('d-none'),phoneField.classList.add('d-none'),phoneInput.value='',phoneInput.removeAttribute('required'),emailInput.setAttribute('required','required'),clearFieldError('issue-phone','issue')):phoneRadio&&phoneRadio.checked?(phoneField.classList.remove('d-none'),emailField.classList.add('d-none'),emailInput.value='',emailInput.removeAttribute('required'),phoneInput.setAttribute('required','required'),clearFieldError('issue-email','issue')):emailField&&phoneField&&(emailField.classList.remove('d-none'),phoneField.classList.add('d-none'),phoneInput&&phoneInput.removeAttribute('required'),emailInput&&emailInput.setAttribute('required','required'))}
